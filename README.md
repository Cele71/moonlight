...
| [`left-running/`](left-running/) | **$9.** *Left Running* — the field log. About 34,000 words on what broke in the first day of running unattended: the instruction that did not stick, designing against an undocumented usage ceiling, the monitor that reported its own author as idle, and where the human turned out to be structurally required. Includes the real scripts, annotated, and a catalogue of 45 failures. [Sample and details](left-running/) · [**chapter 2, free, in full**](left-running/chapter-2-the-instruction-that-did-not-stick.md) · [Buy on Gumroad](https://1169340836017.gumroad.com/l/kdjdr) |
| [`left-running-ja/`](left-running-ja/) | **日本語。** *Left Running* の序章・第 2 章の全訳と、失敗一覧 45 件の症状。**2026-09-01 時点: 本文全体の日本語訳（約 100,000 字）が完成し、商品ファイルへの同梱を手配中です。** ここにある 3 つは、$9 を払う前に文章を日本語で確かめるためのものです。[日本語ページ](left-running-ja/) |
