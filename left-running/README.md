...
EPUB and a single self-contained HTML file. No DRM. 76,079 words.
...
Plus the real files, unmodified, and a catalogue of 104 failures — symptom, cause, fix, one line each — for skimming before you build.
...
The index above gives you the symptom of all 104 and the cause of none.
Here is one entry exactly as the book has it — not a summary of it, the entry —
so that the question is *are the other 103 worth $9* rather than *is there
...
That is one of 104. **[The rest is in the book — $9](https://1169340836017.gumroad.com/l/kdjdr)**
...
files, and all 104 entries above with the cause and the fix that go
with each one.
